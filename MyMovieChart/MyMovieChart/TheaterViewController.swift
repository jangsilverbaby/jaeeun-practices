...
class TheaterViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let coordinatRegion = MKCoordinateRegion(center: location, latitudinalMeters: regionRadius, longitudinalMeters: regionRadius)
        // 5. map 변수에 연결된 지도 객체에 데이터를 전달하여 화면에 표시
        map.setRegion(coordinatRegion, animated: true)
